...
TokenHandler.prototype.handle = function(request, response) {
  ...
  if (request.method === 'POST' && !request.is('application/x-www-form-urlencoded')) {
    return Promise.reject(new InvalidRequestError('Invalid request: if requset method is POST, content must be application/x-www-form-urlencoded'));
  }
...
TokenHandler.prototype.getClient = function(request, response) {
  ...
  let grantType = request.body.grant_type || request.query.grant_type;
...
TokenHandler.prototype.getClientCredentials = function(request) {
  ...
  if (request.query.client_id && request.query.client_secret) {
    return { clientId: request.query.client_id, clientSecret: request.query.client_secret };
  }
